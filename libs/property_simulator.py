...
from libs.chemutils import sanitize, get_mol
from libs.reassemble import NodeMerge, EdgeMerge, merge_node, merge_edge

from libs.sasscore import reward_penalized_log_p
...
class MolPropSearch:
    ...
    def calculate_score(self, cand_mols, objective='logP'):
        ...
        if objective == 'logP':
            ...
            PlogP = np.array([reward_penalized_log_p(mol) for mol in cand_mols])
            scores = PlogP
        else:
    ...
    def greedy_search(self, nodes=[], depth=0, top=10, objective='logP'):
        ...
        nodes = [self.mol_with_atom_index(res, depth) for res in cands if res is not None]
    ...
    def test_combine(self, nodes=[], depth=0):
        ...
        nodes = [self.mol_with_atom_index(res, depth) for res in candidate_mols if res is not None]
        self.tree[depth + 1].append(nodes)
