...
from libs.property_simulator import MolPropSearch
...
class MCTSNode:
    def __init__(self, smiles, W=0, N=0, P=0):
        ...
        self.MW = Descriptors.MolWt(self.mol)  # [Descriptors.MolWt(mol) for mol in self.mols]
...
class MCTS:
    def __init__(self, root_smiles, building_blocks, scoring_function,
                 objective="PlogP", n_choice=1, epsilon=0, max_mw=MAX_MW, max_nodes=MAX_NODES):
        """
    ...
    def rollout(self, node, state_map, scoring_function):
        ...
        for cand_smi in selected_smiles:
            ...
            new_smiles, _ = candidate_smiles(node.smiles, cand_smi, top=self.n_choice, objective=self.objective)
