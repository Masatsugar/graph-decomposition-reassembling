...
from .graph import AUTO_EDGE_ID, VACANT_GRAPH_ID, VACANT_VERTEX_LABEL, Graph
...
class gSpan(object):
    ...
    def _get_support(self, projected):
        """Supportを取得する"""
        _ids = []
        for i in [pdfs.gid for pdfs in projected]:
            _ids.append(i)

        return len(set([pdfs.gid for pdfs in projected])), _ids
    ...
    def _report(self, projected):
        ...
        # 元の DF と連結
        self._report_df = pd.concat([self._report_df, new_row])
        #         if self._visualize:
        #             g.plot()
        if self._where:
            pass
        ...
        for to, e in g.vertices[e2.to].edges.items():
            ...
            if self._is_undirected:
                ...
                if g.vertices[e1.frm].vlb < g.vertices[e2.to] or (
                    g.vertices[e1.frm].vlb == g.vertices[e2.to] and e1.elb <= e.elb
                ):
                    return e
    ...
    def _subgraph_mining(self, projected):
        self._support, self._ids = self._get_support(projected)
