...
def mol_to_svg(
    mol, molSize=(300, 300), kekulize=True, drawer=None, font_size=0.8, **kwargs
):
...
def depict_atoms(
    mol, atom_ids, radii, molSize=(300, 300), atm_color=(0, 1, 0), oth_color=(0.8, 1, 0)
):
...
def plot_class_distribution(
    ...
    h, bins = np.histogram(
        range(len(cmap.colors)), bins=len(c_unique)
    )  # get equally spaced colors from cmap
    colors = [cmap.colors[int(x)] for x in bins[1:]]

    for cl, color in zip(c_unique, colors):
        if len(df[df[c_col] == cl]) > 1:
            sns.kdeplot(
                df[df[c_col] == cl][x_col], ax=h1, c=color, label=cl, legend=False
            )  # hist1
            sns.kdeplot(
                df[df[c_col] == cl][y_col],
                ax=h2,
                c=color,
                vertical=True,
                label=cl,
        ...
        h1.legend(
            handles, labels, bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.0, ncol=n
        )
        sc.scatter(
            df[df[c_col] == cl][x_col],
            df[df[c_col] == cl][y_col],
            c=color,
            marker=marker,
            alpha=alpha,
...
def plot_2D_vectors(
    ...
    if vector_labels:
        ...
        for x, y, u, v, c, vl in zip(X, Y, U, V, colors, vector_labels):
            ...
            ax.quiverkey(
                Q, x, y, u, vl, coordinates="data", color=[0, 0, 0, 0], labelpos="N"
            )
    else:
    ...
    margin_x, margin_y = sum(min_max_x) / 10.0, sum(min_max_y) / 10.0
    ...
    def _get_depictions(self):
        ...
        for idx in self.identifiers:
            for mol, sentence in zip(self.mols, self.sentences):
                if idx in sentence:
                    self.depictions.append(
                        depict_identifier(mol, idx, self.radius, molSize=self.size).data
                    )
                    break
